const checkInventory = async (req, res, next) => {
    ...
    try {
      ...
      const inventoryRequest = {
        email: req.user.email, // Assuming user info is available in the request
        items: cartData.cart.map(item => ({
          productCode: item.product_id, // Using product_id from cart data
          quantity: item.quantity,      // Quantity from cart data
        })),
      };
